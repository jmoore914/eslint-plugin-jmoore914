...
module.exports = {
	...
	create(context) {
		...
		function checkConsequentOrAlternateAssignment(consequentOrAlternateNode){
			...
			consequentOrAlternateNode.body[0].expression.type === 'AssignmentStatement';
		}

		function compareConsequentAndAlternateAssignments(node){
			return node.consequent.body[0].expression.left === node.alternate.body[0].expression.left;
		}
		...
		function checkConsequentAndAlternateCall(node){
			return options.callExp && node.consequent.body[0].type === 'CallExpression';
		}
		...
		function fixFunction(node, fixer){
			...
			if(node.consequent.body[0].type === 'ExpressionStatement'){
				if(compareConsequentAndAlternateAssignments(node)){
					...
					left = node.consequent.body[0].expression.right;
					right = node.alternate.body[0].expression.right;
				}
				else{
					left = node.consequent.body[0].expression;
					right = node.alternate.body[0].expression;
				}
			}
